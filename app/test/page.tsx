...
import { TestCategoriesSkeleton } from "./components/skeletons";
...
function CategoryCard({
  ...
  testResult?: { score: number; percentage: number; createdAt: Date };
}) {
...
async function CategoryList() {
  ...
  // 並行処理：両方のデータフェッチを同時に開始
  const categoriesPromise = getCategories();
  const testResultsPromise = session?.user?.id
  ...
  const { contents: categories } = categoriesData;
    ...
    <>
      {categories.length === 0 ? (
        <div className="bg-white rounded-xl p-8 text-center max-w-md mx-auto border border-gray-100">
          ...
          <p className="text-gray-500">テストカテゴリが見つかりませんでした</p>
        </div>
      ) : (
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {basicCategories.map((category) => (
                  <CategoryCard
                    key={category.id}
                    category={category}
                    testResult={completedTests[category.id]}
                  />
                ))}
              </div>
            </div>
